...
class StonesCanMove:
    ...
    def __get_moves__(self):  # kendi taşlarının konumlarını alır
        ...
        for soldier in soldiers:
            ...
            if soldier.x > 1 and (
                    self.matrix[soldier.y][soldier.x - 1] == 'w' or self.matrix[soldier.y][soldier.x - 1] == 'W'):
            ...
            if soldier.y > 1 and (
                    self.matrix[soldier.y - 1][soldier.x] == 'w' or self.matrix[soldier.y - 1][soldier.x] == 'W'):
                # arkası da boş olması lazım
...
class Node:
    ...
    # def print_state_reverse(self):
    #     char_array = list(self.state)
...
class NodeDao:
    ...
    def __create_node_table(self):
        ...
        # first_state = '00000000000000000000w000w000m00wm000000m00000000m00000000000M000'
    ...
    def get_by_state(self, state: int):
    ...
    def save_experience(self, node: Node):
...
class BranchMaker:
    ...
    def create_next_nodes(node: Node):
        ...
        if stones_can_move.commander_forced_moves or stones_can_move.soldier_forced_moves:
            commander_attack_result = BranchMaker.__commander_attacks(BranchMaker.matrix,
                                                                      stones_can_move.commander_forced_moves)
            soldier_attack_result = BranchMaker.__soldier_attacks(BranchMaker.matrix,
                                                                  stones_can_move.soldier_forced_moves)
            ...
            for i in range(stone.x - 1, 0, -1):
                if matrix[stone.y][i] == 'w' or matrix[stone.y][i] == 'W':
                    if matrix[stone.y][i - 1] == '0':
                        ...
                        attack_list.append(Attack(new_matrix, kill_count))
                        BranchMaker.__commander_attack(new_matrix, Coordinate(stone.y, i - 1), attack_list,
                                                       kill_count + 1)
                        for j in range(i - 2, -1, -1):
                            if matrix[stone.y][j] == '0':
                                ...
                                attack_list.append(Attack(new_matrix, kill_count))
                                BranchMaker.__commander_attack(new_matrix, Coordinate(stone.y, j), attack_list,
                                                               kill_count + 1)
                            else:
                                break
                        break
            ...
            for i in range(stone.y - 1, 0, -1):
                if matrix[i][stone.x] == 'w' or matrix[i][stone.x] == 'W':
                    if matrix[i - 1][stone.x] == '0':
                        ...
                        attack_list.append(Attack(new_matrix, kill_count))
                        BranchMaker.__commander_attack(new_matrix, Coordinate(i - 1, stone.x), attack_list,
                                                       kill_count + 1)
                        for j in range(i - 2, -1, -1):
                            if matrix[j][stone.x] == '0':
                                ...
                                attack_list.append(Attack(new_matrix, kill_count))
                                BranchMaker.__commander_attack(new_matrix, Coordinate(j, stone.x), attack_list,
                                                               kill_count + 1)
                            else:
                                break
                        break
...
# dipten yukarıya doğru durum değerlerini hesaplasın
...
def computer_move_calculater(root_node):
    ...
    if id is not None:
        ...
        if root_node.result is not None:
            ...
            if move_count % 2 == 0:
                win = 2
            child_nodes = node_dao.get_children_of_father(root_node.id)
            # kazanma durumu
            ...
            for child_node in child_nodes:
                if child_node.result == root_node.result:
                    return child_node
        # bu düğüm veri tabanında var ama sonucu belli değil, o zaman çocukları veri tabanında olabilir.
    ...
    # oyun sunucunu kontrol et ve tecrubeleri kaydet
    if root_node.result is not None:
        # beraberlik durumu
        if root_node.result == 1:
            node_dao.save_draw(root_node)
        else:
            global user_turn
            computer_win = user_turn * 2
            # bilgisayarın kazanması durumu
            if root_node.result == computer_win:
                node_dao.save_win(root_node)
            # bilgisayarın kaybetmesi durumu
            else:
                node_dao.save_lose(root_node)
    global state_list
    for next_node in next_nodes:
        if next_node.state_value == root_node.state_value and next_node.state not in state_list:
            return next_node
    # en iyi hamleyi bulamadı, döngüden dolayı...
...
def computer_move(root_node):
    next_node = computer_move_calculater(root_node)
...
node_dao.save_experience(current_node)
